...
class DataFetcher:
    ...
    def fetch_all_timeframes(self,symbol:str) -> Dict[str,pd.DataFrame]:
        ...
        for i, timeframe in enumerate(timeframes):
            ...
            # Checks to see if timeframe for current symbol exists in the cache
            ...
            # Will add an interval timer to abide by IBKR fetching rules if...
            # - there exists more timeframes to fetch
            # - previous fetched data was not fetched from cache
            # - an api connection was made to IBKR
            if i < len(timeframes) - 1 and not was_cached  and self.connected:
    ...
    def _cache_exists(self,symbol:str,timeframe:str) -> bool:
        ...
        lookback_period = self.data_config.lookback_period.replace(' ', '')
        cache_file = self.cache_dir / f"{symbol}_{timeframe}_{lookback_period}.csv"
    ...
    def fetch_historical_data(self,symbol:str, timeframe:str) -> pd.DataFrame | None:
        ...
        if not self.connected:
            self.logger.warning("Not connected to IB. Connecting to IBKR...")
            if not self.connect():
                return None
        ...
        # Convert IB BarData objects to DataFrame
        df = util.df(all_bars)
    ...
    def _load_from_cache(self, symbol: str, timeframe:str) -> pd.DataFrame | None:
        ...
        if not cache_file.exists():
            self.logger.debug(f"No cache found for {symbol}_{timeframe}_{lookback_period}")
            return None

        try:
            # Read CSV and parse dates
            df = pd.read_csv(cache_file, parse_dates=['date'])
            self.logger.info(f"Loaded {symbol}_{timeframe}_{lookback_period} from cache ({len(df)} bars)")
            return df

        except Exception as e:
            self.logger.error(f"Failed to load cache for {symbol}_{timeframe}_{lookback_period}: {e}")
            return None
    ...
    def _fetch_bars_chunked(self,contract:Stock,bar_size:str,symbol:str,timeframe:str) -> list:
        ...
        Returns all bars as an array where each bar is an IB BarData object"""
    ...
    def _save_to_cache(self, symbol: str, df: pd.DataFrame, timeframe:str) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Cached {symbol}_{timeframe}_{lookback_period} ({len(df)} bars) to {cache_file}")
            return True

        except Exception as e:
            self.logger.error(f"Failed to cache {symbol}_{timeframe}_{lookback_period}: {e}")
            return False
