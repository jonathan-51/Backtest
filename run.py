...
def _inject_regime_data(fetcher: DataFetcher,all_data: dict) -> None:
    """Fetch regime ETF data and inject it into every symbol's data dict."""
    
    # Fetch daily bars for each regime scoring ETF (SPY, QQQ, HYG, etc.)
    regime_data = {}
    ...
    # Attach regime data to each symbol so strategies can access it
    for symbol in all_data:
        all_data[symbol]['regime_data'] = regime_data

        # ATRChannelBreakout reads spy_1d directly for its own SMA regime filter
